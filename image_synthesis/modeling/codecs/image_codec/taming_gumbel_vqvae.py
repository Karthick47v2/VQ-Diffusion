...
class Decoder(nn.Module):
    ...
    def forward(self, indices):
        z = self.quantize.get_codebook_entry(
            indices.view(-1), shape=(indices.shape[0], self.h, self.w, -1))
...
class TamingFFHQVQVAE(BaseCodec):
    def __init__(
        ...
        self.dec = Decoder(model.decoder, model.post_quant_conv,
                           model.quantize, token_shape[0], token_shape[1])
...
class TamingGumbelVQVAE(BaseCodec):
    ...
    def get_tokens(self, imgs, **kwargs):
        ...
        code = self.enc(imgs)
        if self.quantize_number != 0:
            # print(self.full_to_quantize.device, code.device)
            code = self.full_to_quantize.to(imgs.device)[code]
        output = {'token': code}
        # output = {'token': rearrange(code, 'b h w -> b (h w)')}
        return output
